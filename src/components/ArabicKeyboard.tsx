    ...
    <div id="arabic-keyboard" className="w-full max-w-2xl mx-auto px-1 select-none touch-manipulation">
      ...
      {onUseJoker && (
        <div className="flex items-center justify-between pb-1.5 px-2">
          <button
            ...
            type="button"
            disabled={disabled || jokerDisabled || jokersAvailable <= 0}
            onClick={onUseJoker}
          ...
          >
            ...
            <span>الجوكر 🃏 ({jokersAvailable} متبقي)</span>
      ...
      <div className="flex flex-col gap-1.5 sm:gap-2">
        {ARABIC_KEYBOARD_LAYOUT.map((row, rowIndex) => (
          <div key={rowIndex} className="flex justify-center items-center gap-1 sm:gap-1.5 w-full">
            {row.map((key) => {
              ...
              if (isEnter) {
                return (
                  <button
                    ...
                    className="flex-1 max-w-[4.5rem] h-11 sm:h-13 rounded-xl font-bold text-xs sm:text-sm bg-emerald-500/90 hover:bg-emerald-400 active:scale-95 text-white flex items-center justify-center gap-1 transition-all disabled:opacity-40 disabled:pointer-events-none shadow-lg shadow-emerald-500/25 border border-emerald-300/30 backdrop-blur-md cursor-pointer"
              ...
              if (isBackspace) {
                return (
                  <button
                    ...
                    className="flex-1 max-w-[4rem] h-11 sm:h-13 rounded-xl font-bold text-xs sm:text-sm bg-white/[0.08] hover:bg-white/[0.16] active:scale-95 text-white/90 flex items-center justify-center transition-all disabled:opacity-40 disabled:pointer-events-none shadow-xs border border-white/10 backdrop-blur-md cursor-pointer"
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-7 sm:min-w-9 h-11 sm:h-13 rounded-xl font-bold text-base sm:text-lg border flex items-center justify-center transition-all active:scale-90 disabled:opacity-40 disabled:pointer-events-none cursor-pointer ${getKeyColor(
                    key
                  )}`}
                  aria-label={`حرف ${key}`}
                >
