...
export const GameBoard: React.FC<GameBoardProps> = ({
    ...
    for (let colIndex = 0; colIndex < wordLength; colIndex++) {
      ...
      tiles.push(
        <div
          ...
          style={animStyle}
          className={`relative overflow-hidden w-11 h-11 sm:w-13 sm:h-13 rounded-2xl flex items-center justify-center text-xl sm:text-2xl font-bold select-none transition-colors duration-200 ${bgClass} ${animClass}`}
        >
  ...
  return (
    <div id="game-board-container" className="flex flex-col gap-1.5 sm:gap-2 justify-center my-auto py-1">
      {rows}
    </div>
  );
};
